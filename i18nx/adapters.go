...
func (a *FileAdapter) Load(ctx context.Context) (map[string]map[string]any, error) {
	...
	select {
	case <-ctx.Done():
		return nil, fmt.Errorf("loading translation file '%s' canceled: %w", a.path, ctx.Err())
	case <-done:
	...
	if readErr != nil {
		return nil, fmt.Errorf("failed to read translation file '%s': %w", a.path, readErr)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse translation file '%s': %w", a.path, err)
	}
...
func (a *DirectoryAdapter) Load(ctx context.Context) (map[string]map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to access directory '%s': %w", a.path, err)
	}
	if !fileInfo.IsDir() {
...
func (a *DirectoryAdapter) processDirectory(ctx context.Context, allTranslations map[string]map[string]any) error {
	...
	select {
	case <-ctx.Done():
		return fmt.Errorf("loading from directory '%s' canceled: %w", a.path, ctx.Err())
	case <-done:
	...
	if readErr != nil {
		return fmt.Errorf("failed to read directory '%s': %w", a.path, readErr)
	}
	...
	for _, entry := range entries {
		...
		if ctx.Err() != nil {
			return fmt.Errorf("context canceled while processing directory '%s': %w", a.path, ctx.Err())
		}
...
func (a *DirectoryAdapter) processFile(ctx context.Context, filePath string, allTranslations map[string]map[string]any) error {
	...
	select {
	case <-ctx.Done():
		return fmt.Errorf("loading translation file '%s' canceled: %w", filePath, ctx.Err())
	case <-done:
		// Continue with normal processing
	}
	...
	if readErr != nil {
		return fmt.Errorf("failed to read translation file '%s': %w", filePath, readErr)
	}
...
func (a *EmbeddedFsAdapter) processFile(ctx context.Context, filePath string, allTranslations map[string]map[string]any) error {
	...
	select {
	case <-ctx.Done():
		return fmt.Errorf("loading embedded translation file '%s' canceled: %w", filePath, ctx.Err())
	case <-done:
		// Continue with normal processing
	}
